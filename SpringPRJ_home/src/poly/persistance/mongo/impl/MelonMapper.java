...
public class MelonMapper implements IMelonMapper {
	...
	public boolean createCollection(String colNm) throws Exception {
		...
		// 데이터 양이 많지 않으나, 최소 10만건 이상 데이터 저장시 속도가 약 10배 이상 발생함
	...
	public int insertRank(List<MelonDTO> pList, String colNm) throws Exception {
		...
		log.info(getClass().getName() + ".insertRank Start!");
	...
	public List<MelonDTO> getRank(String colNm) throws Exception {
		...
		// 데이터를 가져올 컬렉션 선택 DBCollection rCol = mongodb.getCollection(colNm);
		DBCollection rCol = mongodb.getCollection(colNm);

		// 컬렉션으로부터 전체 데이터 가져오기 Iterator<DBObject> cursor = rCol.find();
		Iterator<DBObject> cursor = rCol.find();

		// 컬렉션으로부터 전체 데이터 가져온 것을 List 형태로 저장하기 위한 변수 선언 List<MelonDTO> rList = new
		List<MelonDTO> rList = new ArrayList<MelonDTO>();

		// 퀴즈팩별 정답률 일자별 저장하기 MelonDTO rDTO = null;
		MelonDTO rDTO = null;
		...
		while (cursor.hasNext()) {
			...
			String album = CmmUtil.nvl((String) current.get("album")); // 앨범
